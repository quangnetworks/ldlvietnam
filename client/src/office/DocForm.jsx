...
import { DOC_KINDS } from '../utils.js';
...
const EMPTY = {
  ...
  need_numbering: false, approvers: [], recipient_users: [], recipient_departments: [], followers: [],
};
...
export default function DocForm() {
  ...
  useEffect(() => {
    ...
    api.get(`/documents/${id}`).then((d) => {
      ...
      setForm({
        ...
        followers: d.followers.map((f) => f.id),
      });
  ...
  const save = async (mode) => {
    ...
    setBusy(true);
    try {
      const body = { ...form, need_numbering: form.need_numbering ? 1 : 0 };
      let doc;
      if (id) {
