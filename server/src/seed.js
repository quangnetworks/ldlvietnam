...
import { db, run, get, tx, setSetting, logActivity } from './db.js';
...
export function seed({ reset = false } = {}) {
  tx(() => {
    if (reset) for (const t of TABLES) db.exec(`DELETE FROM ${t}`);
    setSetting('company_name', 'CÔNG TY CỔ PHẦN LDL VIỆT NAM');

    // ---------- Phòng ban
    const dep = {};
    for (const [key, name, code] of [
      ['bgd', 'Ban Giám đốc', 'BGD'], ['hcns', 'Phòng Hành chính Nhân sự', 'HCNS'], ['kd', 'Phòng Kinh doanh', 'KD'],
      ['mkt', 'Phòng Marketing', 'MKT'], ['kt', 'Phòng Kế toán', 'KT'], ['it', 'Phòng IT', 'IT'],
    ]) dep[key] = Number(run('INSERT INTO departments(name, code) VALUES (?,?)', name, code).lastInsertRowid);

    // ---------- Người dùng (mật khẩu mặc định 123456)
    const hash = bcrypt.hashSync('123456', 10);
    const users = {};
    const addUser = (key, username, name, title, depKey, role = 'member', manager = null) => {
      users[key] = Number(run(`INSERT INTO users(username, password_hash, name, email, title, department_id, role, color, manager_id)
        VALUES (?,?,?,?,?,?,?,?,?)`, username, hash, name, `${username}@ldlvietnam.vn`, title, dep[depKey], role,
      COLORS[Object.keys(users).length % COLORS.length], manager ? users[manager] : null).lastInsertRowid);
    };
    ...
    // ---------- Office: danh mục
    const types = {};
    for (const [k, n, p] of [['cs', 'Chính sách', 'CS'], ['ct', 'Chỉ thị', 'CT'], ['cv', 'Công văn', 'CV'],
      ['hd', 'Hợp đồng dịch vụ', 'HĐ'], ['qd', 'Quyết định', 'QĐ'], ['tb', 'Thông báo', 'TB']]) {
      types[k] = Number(run('INSERT INTO doc_types(name, prefix) VALUES (?,?)', n, p).lastInsertRowid);
    }
    const folder = (name, parent = null) => Number(run('INSERT INTO doc_folders(name, parent_id) VALUES (?,?)', name, parent).lastInsertRowid);
    ...
    const cat = (name, parent = null) => Number(run('INSERT INTO doc_categories(name, parent_id) VALUES (?,?)', name, parent).lastInsertRowid);
    cat('Chưa phân loại');
    ...
    // ---------- Office: văn bản mẫu
    const addDoc = (d) => {
      const id = Number(run(`INSERT INTO documents(code, title, description, content, kind, type_id, folder_id, category_id,
        ...
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      d.code ?? null, d.title, d.description ?? null, d.content ?? null, d.kind, d.type, d.folder ?? null, d.category ?? null,
      d.department ?? null, d.sender ?? null, d.issuer, d.creator ?? d.issuer, d.status,
      d.status === 'issued' ? datetimeOffset(d.ago ?? 0) : null, d.effective ?? null, d.expire ?? null,
      datetimeOffset(d.ago ?? 0), datetimeOffset(d.ago ?? 0), d.numbering ? 1 : 0).lastInsertRowid);
      (d.approvers || []).forEach((u, i) => run('INSERT INTO document_approvers(document_id, user_id, step, status, acted_at) VALUES (?,?,?,?,?)',
        id, u, i + 1, d.status === 'issued' ? 'approved' : 'pending', d.status === 'issued' ? datetimeOffset(d.ago ?? 0) : null));
      for (const v of d.viewers || []) run('INSERT OR IGNORE INTO document_views(document_id, user_id) VALUES (?,?)', id, v);
      run('INSERT OR IGNORE INTO document_follows(document_id, user_id) VALUES (?,?)', id, d.creator ?? d.issuer);
      logActivity('document', id, d.creator ?? d.issuer, 'created', 'Tạo văn bản');
      if (d.status === 'issued') logActivity('document', id, d.issuer, 'issued', 'Văn bản đã được ban hành');
      return id;
    };
    addDoc({
    ...
    const addProject = (name, kind, owner, members, color, lists, group = null) => {
      const id = Number(run('INSERT INTO projects(name, kind, owner_id, color, group_name, description) VALUES (?,?,?,?,?,?)',
        name, kind, owner, color, group, `Không gian làm việc của ${name}`).lastInsertRowid);
      run("INSERT INTO project_members(project_id, user_id, role) VALUES (?,?, 'manager')", id, owner);
      for (const m of members) run("INSERT OR IGNORE INTO project_members(project_id, user_id, role) VALUES (?,?, 'member')", id, m);
      const listIds = lists.map((l, i) => Number(run('INSERT INTO task_lists(project_id, name, position) VALUES (?,?,?)', id, l, i + 1).lastInsertRowid));
      return { id, lists: listIds };
    };
    ...
    const tpl = addProject('Mẫu: Ra mắt sản phẩm', 'project', users.admin, [], '#1098ad', ['Nghiên cứu', 'Chuẩn bị', 'Ra mắt']);
    run('UPDATE projects SET is_template = 1 WHERE id = ?', tpl.id);

    const addTask = (t) => {
      const id = Number(run(`INSERT INTO tasks(project_id, list_id, parent_id, title, description, creator_id, assignee_id, status, priority,
          start_date, due_date, completed_at, recurring, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
      t.project ?? null, t.list ?? null, t.parent ?? null, t.title, t.description ?? null, t.creator, t.assignee, t.status ?? 'todo',
      t.priority ?? 'normal', t.start ?? null, t.due ?? null, t.status === 'done' ? datetimeOffset(t.doneAgo ?? 0) : null,
      t.recurring ?? null, datetimeOffset(t.ago ?? 0), datetimeOffset(t.ago ?? 0)).lastInsertRowid);
      for (const f of t.followers || []) run('INSERT OR IGNORE INTO task_followers(task_id, user_id) VALUES (?,?)', id, f);
      for (const [i, c] of (t.checklist || []).entries()) run('INSERT INTO task_checklist(task_id, content, done, position) VALUES (?,?,?,?)', id, c[0], c[1] ? 1 : 0, i);
      logActivity('task', id, t.creator, 'created', 'Tạo công việc');
      return id;
    };
    ...
    run('INSERT INTO goals(user_id, title, progress, due_date) VALUES (?,?,?,?)', users.nv1, 'Đạt doanh số 2 tỷ Quý 4', 35, dateOffset(90));
    run("INSERT INTO notifications(user_id, actor_id, app, type, title, link) VALUES (?,?, 'wework', 'assigned', ?, '/wework')",
...
if (process.argv[1] && fileURLToPath(import.meta.url) === path.resolve(process.argv[1])) {
  ...
  console.log(`Đã tạo dữ liệu mẫu (${n} người dùng). Đăng nhập: admin / 123456 hoặc demo / 123456`);
}
